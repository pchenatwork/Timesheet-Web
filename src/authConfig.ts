...
const getConfig = (aadConfig: any, b2cConfig: any): any => {
  ...
  switch (process.env.REACT_APP_IDENTITY_PROVIDER) {
    ...
    default:
      throw new Error(
        `'${process.env.REACT_APP_IDENTITY_PROVIDER}' is not a recognized identity provider.`
      );
  }
};
...
export const getLoginRequest = () =>
  getConfig(loginRequest_AAD, loginRequest_B2C);

export const getScopes = () =>
  getConfig(loginRequest_AAD.scopes, loginRequest_B2C.scopes);
